...
| Pattern | Status | What it does | Tags |
|---|---|---|---|
| [grain-texture](grain-texture/) | production | Body-wide film-grain noise overlay via `body::before` | texture, global, css-only, no-js |
| [coverflow-doors-tuner](coverflow-doors-tuner/) | experiment | 3-door scroll-driven coverflow carousel with arch wave + live config tuner | layout, hero, carousel, svg, canvas, animation, scroll-driven, 3d |
...
4. **Drop in `example.html`** — a standalone, runnable demo. No build step,
   no external deps. Double-click to view.
...
6. **Add a row to the table above.** Status starts at `experiment`; promote
   to `production` after a second successful reuse.
